...
async def modify_workspace(request: Request):
    ...
    # Check credits (modification costs 50 credits)
    MODIFY_COST = 50
    is_unlimited = user_doc.get("unlimited_credits", False) or user_doc.get("is_owner", False)
    ...
    if not is_unlimited and user_doc["credits"] < MODIFY_COST:
        raise HTTPException(
            status_code=402,
            detail=f"Modificación requiere {MODIFY_COST} créditos. Tienes {user_doc['credits']}."
        )
    ...
    await manager.broadcast(workspace_id, {
        ...
        "log": {
            "agent": "system",
            "type": "working",
            "message": f"Procesando modificación...",
            "timestamp": utc_now().isoformat()
        }
    })
    
    # Use frontend agent to modify files based on instruction
    modify_prompt = f"""Modify the existing code based on this instruction:

INSTRUCTION: {instruction}

EXISTING FILES:
{json.dumps(list(files.keys()), indent=2)}

IMPORTANT:
- Output the modified files in JSON format
...
{{
    "files": {{
        "src/path/to/file.jsx": "COMPLETE MODIFIED CODE"
    }},
    ...
    try:
            ...
            {"workspace_id": workspace_id},
            {
                ...
                "$push": {
                    "versions": {
                        ...
                        "files": files,
                        "created_at": utc_now(),
                        "message": f"Modificación: {instruction[:50]}"
                    }
                }
            }
        )
        ...
        await manager.broadcast(workspace_id, {
            ...
            "log": {
                "agent": "system",
                "type": "success",
                "message": f"Modificación aplicada ({len(modified_files)} archivos)",
                "timestamp": utc_now().isoformat()
            }
        })
...
async def modify_workspace(request: Request):
    ...
    await manager.broadcast(workspace_id, {
        ...
        "log": {
            "agent": "system",
            "type": "working",
            "message": "Procesando modificación...",
            "timestamp": utc_now().isoformat()
        }
    })
