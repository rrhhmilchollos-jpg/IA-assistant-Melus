...
async def github_login(request: Request):
    ...
    external_url = os.environ.get('EXTERNAL_URL', 'https://ai-app-generator-6.preview.emergentagent.com')
...
async def github_callback(request: Request, code: str, state: str):
    ...
    frontend_url = os.environ.get('FRONTEND_URL', 'https://ai-app-generator-6.preview.emergentagent.com')
...
---
Generated on {utc_now().strftime('%Y-%m-%d %H:%M:%S')} by [Assistant Melus](https://ai-app-generator-6.preview.emergentagent.com)
"""
...
@router.post("/push-workspace")
async def push_workspace_to_github(request: Request):
    """Push a workspace project to GitHub - New implementation for WorkspacePage"""
    db = request.app.state.db
    user_doc = await get_authenticated_user(request, db)
    user_id = user_doc["user_id"]
    ...
    PUSH_COST = 50  # 50 credits to push to GitHub
    
    if not is_unlimited and user_doc["credits"] < PUSH_COST:
        raise HTTPException(status_code=402, detail=f"Necesitas {PUSH_COST} créditos para subir a GitHub")
    
    connection = await db.github_connections.find_one({"user_id": user_id})
    if not connection:
        raise HTTPException(status_code=400, detail="GitHub no conectado. Ve a configuración para conectar tu cuenta.")
    ...
    if not workspace_files:
        raise HTTPException(status_code=400, detail="No hay archivos para subir")
...
## Generado con Melus AI
Este proyecto fue generado usando arquitectura multi-agente con IA.
...
```
{chr(10).join([f'├── {f}' for f in list(workspace_files.keys())[:10]])}
```

---
Generado el {utc_now().strftime('%Y-%m-%d %H:%M:%S')} por [Melus AI](https://melusai.com)
"""
    files.append({"path": "README.md", "content": readme_content})
    ...
    try:
        ...
        final_repo_name = repo_name or workspace.get("name", "melus-app").lower()
        final_repo_name = "".join(c if c.isalnum() or c == "-" else "-" for c in final_repo_name)
        final_repo_name = final_repo_name[:50]  # GitHub repo name limit
        ...
        if create_new:
            try:
                repo_info = manager.create_repository(
                    name=final_repo_name,
                    description=workspace.get("description", "Generated by Melus AI")[:200],
                    private=private
                )
                logger.info(f"Created repo: {final_repo_name}")
            except Exception as create_error:
                if "already exists" in str(create_error).lower():
                    logger.info(f"Repo {final_repo_name} already exists, will push to it")
                else:
                    raise create_error
        ...
        push_result = manager.push_files(
            repo_name=final_repo_name,
            files=files,
            commit_message=f"🚀 {workspace.get('name', 'App')} - Generated by Melus AI"
        )
        ...
        return {
            ...
            "repo_url": push_result.get("repo_url"),
            "files_pushed": push_result.get("total_files"),
            "credits_used": 0 if is_unlimited else PUSH_COST,
            "credits_remaining": new_credits
        }
        
    except Exception as e:
        logger.error(f"GitHub push workspace error: {e}")
        raise HTTPException(status_code=500, detail=str(e))
